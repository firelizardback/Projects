...
var doBFS = function(graph, source) {
    ...
    while (!queue.isEmpty())    {
        ...
        for (var i=0;i < graph[vert].length; i++) {
            var verti = graph[vert][i];
            if (bfsInfo[verti].distance === null) {
                bfsInfo[verti].distance = bfsInfo[vert].distance + 1;
                bfsInfo[verti].predecessor = vert;
                queue.enqueue(verti);
            }
        }
    }
